...
func NewRouter(userHandler *handler.UserHandler, storeHandler *handler.StoreHandler, productHandler *handler.ProductHandler, middleware middleware.AuthMiddleware) *httprouter.Router {
	...
	router.PATCH("/api/stores", middleware.AuthMiddleware(storeHandler.Update))
	...
	router.GET("/api/stores/products/:productId", middleware.AuthMiddleware(productHandler.FindById))
	router.PATCH("/api/stores/products/:productId", middleware.AuthMiddleware(productHandler.Update))
	router.DELETE("/api/stores/products/:productId", middleware.AuthMiddleware(productHandler.Delete))
